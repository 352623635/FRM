  ...
  <div class="line">
    <div>
      <div class="link">
        <div class="logo">
          <a :href=logo.href>
            <div style="width: 210px;height: 70px" :style="{backgroundImage: 'url('+logo.url+')',backgroundSize:'100%',width:'100%',height:'100%'}"></div>
          </a>
        </div>
        <div class="link-father">
          <div :style="{width:n}" class="link-child" v-for="(link,index) in path" :key="index.link">
            <div style="width: 100%" v-if="link.url !== 0">
              <!--          传入时有包含子路径则设置为0-->
              <!--            栏目宽度控制区，默认100%-->
        ...
        <div class="tab-log-box" v-if="href!='#/login'&&href!='#/register'">
          <div class="to-log" v-if=!session.data>
            <a href="#/login">
              <el-button>登录</el-button></a>
          </div>
          <div class="to-logout" v-else>
            <div >
      ...
      <div class="tab-search" v-if="href!='#/login'&&href!='#/register'">
        <label>
